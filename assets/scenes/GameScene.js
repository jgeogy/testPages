...
class GameScene extends Phaser.Scene {
	...
	game_BG;			//to reference game BG
	game_Items;			//to reference the objects behind which the numbers hide
	joury;				//joury reference
	adam;				//adam reference
	levelName;			//Level number 1,2,3 object reference for tweening
	nameBG;				//the circle on which level number is shown
	gameTimer;			//timer for play time limit
	timerMask;			//mask for timer image
	hiddenNum;			//number to find
	falseHiddenNum1;	//wrong hidden number 1
	falseHiddenNum2;	//wrong hidden number 2
	currentNum;			//current number being shown
	numTimer;			//timer for number wait time
	maxGameTime;		//Time limit for the game
	maxVisibleTime;		//Time limit for number to be shown
	numClickable;		//Is the number clickable
	score;				//Count number of hidden numbers clicked
	wrongClickCount;	//Number of wrong clicks
	rightNumCount;		//Number of correct number displayed
	wrongNumCount;		//Number of incorrect number displayed
	gamePause;			//for referencing game pause state
	//Tween distances 1=80, 2=120, 3=100
	firstTweenX;		//Distance for hidden number to tween
	secondTweenX;		//Distance for falseHiddenNum1 to tween
	thirdTweenX;		//Distance for falseHiddenNum2 to tween
	sceneRef;			//Reference to this scene
	bgmRef;				//Reference to BGM player
